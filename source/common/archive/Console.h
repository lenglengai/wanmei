...
namespace std {
	...
	class __funapi Console : boost::noncopyable
	{
	public:
	...
	template<class T>
	void Console::runStream(T& nValue, const char * nName)
	{
		nValue.serialize(this, 0);
	}

	template<class T>
	void Console::runStreamCount(T& nValue, const char * nName, __i32 nCount)
	{
		nValue.serialize(this, nCount);
	}

	template<class T0>
	void Console::runStreamsCount(std::list<std::shared_ptr<T0> >& nValue, const char * nNames, const char * nName, __i32 nCount)
	{
	...
	template<class T0, class T1>
	void Console::runKeyStreamsCount(map<T0, std::shared_ptr<T1>>& nValue, const char * nNames, const char * nName, __i32 nCount)
	{
		__i16 count_ = static_cast<__i16>(nValue.size());
		this->runInt16(count_, "count");
